...
import { Check, Power, RefreshCcw, Timer, Zap } from "lucide-react";
...
export function MaintenanceSettingsView({ snapshot, saving, onSaveSettings, onPowerAction }: { snapshot: MaintenanceSnapshot; saving: boolean; onSaveSettings: (changes: Partial<MaintenanceSnapshot["settings"]>) => Promise<void>; onPowerAction: (action: "reboot" | "poweroff") => Promise<void> }) {
  const [section, setSection] = useState<"appearance" | "session" | "power">("appearance");
  ...
  return (
    <section className="grid min-h-0 grid-rows-[3.5rem_1fr] gap-3 overflow-hidden">
      <div className="grid w-[39rem] grid-cols-[1fr_1.25fr_1fr] rounded-[0.65rem] bg-surface p-1" role="tablist" aria-label="Catégories de réglages"><SettingsTab selected={section === "appearance"} onClick={() => setSection("appearance")}>Apparence</SettingsTab><SettingsTab selected={section === "session"} onClick={() => setSection("session")}>Écran & session</SettingsTab><SettingsTab selected={section === "power"} onClick={() => setSection("power")}>Alimentation</SettingsTab></div>
      ...
      </div> : section === "session" ? <div className="grid content-start grid-rows-[repeat(2,7.5rem)] gap-3">
      ...
      </div> : <div className="grid min-h-0 grid-cols-2 gap-4">
        <PowerAction disabled={!snapshot.power_available} title="Redémarrer la borne" detail="La borne s’arrête, redémarre puis revient automatiquement au kiosque." icon={<RestartIcon />} onClick={() => { setPowerError(null); setConfirmation("reboot"); }} />
        <PowerAction destructive disabled={!snapshot.power_available} title="Éteindre la borne" detail="La borne s’arrête complètement. Un rallumage physique sera nécessaire." icon={<Power className="size-20" strokeWidth={1.8} />} onClick={() => { setPowerError(null); setConfirmation("poweroff"); }} />
        <p className={`col-span-2 text-center text-base ${powerError ? "text-warn" : "text-muted"}`} role="status">{powerError ?? (snapshot.power_available ? "Ces actions sont disponibles uniquement lorsque la borne est au repos." : "Actions indisponibles hors Raspberry Pi avec systemd.")}</p>
      </div>}
...
function SettingsTab({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: string }) {
  return <button type="button" role="tab" aria-selected={selected} className="min-h-12 rounded-[0.45rem] font-semibold text-muted aria-selected:bg-[#d8dee4] aria-selected:text-[#101418]" onClick={onClick}>{children}</button>;
}

function PowerAction({ title, detail, icon, disabled, destructive = false, onClick }: { title: string; detail: string; icon: ReactNode; disabled: boolean; destructive?: boolean; onClick: () => void }) {
  return <button type="button" disabled={disabled} onClick={onClick} className={`group grid min-h-0 place-items-center content-center gap-4 rounded-[0.65rem] border-2 bg-surface px-8 py-6 text-center transition-transform duration-150 active:scale-[0.985] disabled:cursor-not-allowed disabled:opacity-40 ${destructive ? "border-warn text-warn" : "border-signal text-signal"}`}><span className="grid size-24 place-items-center">{icon}</span><span><strong className="block text-[2rem] leading-none text-current">{title}</strong><small className="mx-auto mt-3 block max-w-[24rem] text-lg leading-snug font-normal text-muted">{detail}</small></span></button>;
}

function RestartIcon() {
  return <span className="relative grid size-24 place-items-center" aria-hidden="true"><RefreshCcw className="size-20" strokeWidth={1.7} /><Power className="absolute size-8" strokeWidth={2.2} /></span>;
}
