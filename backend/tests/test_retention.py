...
def test_purge_par_plafond_d_espace(sessions_root: Path) -> None:
    ...
    removed = purge(sessions_root, RetentionPolicy(max_age_days=30, max_total_bytes=2500), now=NOW)

    # Les plus anciennes partent en premier, jusqu'à repasser sous le plafond.
    assert removed == ["s0", "s1"]
    assert sum(len(f.read_bytes()) for f in sessions_root.rglob("*.jpg")) <= 2500
...
def test_un_repertoire_absent_ne_fait_pas_echouer(tmp_path: Path) -> None:
    ...
    removed = purge(tmp_path / "jamais-cree", RetentionPolicy(max_age_days=30, max_total_bytes=0))
